...
**EVALUACIÓN PRÁCTICA FINAL DE UNIDAD - SEGURIDAD INFORMÁTICA**
...
> **Nota:** a mitad de este laboratorio se cambió a otra cuenta de AWS Academy, ya que la cuenta utilizada en los Labs 1 y 2 dejó de estar disponible al finalizarse el curso de AWS Academy. Por esta razón, la instancia `lab3-gutierrezanco` fue creada desde una cuenta distinta a la de los laboratorios anteriores.
...
**Análisis de las Top 10 anomalías**
...
1. Ve a **Discover** en el menú lateral.
2. Selecciona el Data View `wazuh-alerts-4.x-*`.
3. Filtra por **"Last 24 hours"** en la esquina superior derecha.
4. Explora los eventos visibles en la interfaz.
#### 3. Exportar 20 eventos a CSV
 
> ⚠️ **Nota de seguridad:** la contraseña real de tu clúster nunca debería quedar en texto plano en un README público. Se reemplazó abajo por `<TU_PASSWORD_AQUI>`; te recomiendo rotarla si la instancia sigue activa.
 
```bash
# Desde la terminal de la instancia
curl -k -u admin:'<TU_PASSWORD_AQUI>' \
...
2. Nombre: `SOC - Monitor de Seguridad`.
3. Agrega las 4 visualizaciones desde **"Add from library"**.
4. Configura el filtro de tiempo: **Last 24 hours**.
